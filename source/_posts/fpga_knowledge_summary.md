...
**查找表 (LUT)** ，本质上就是一个 RAM 。目前 FPGA 中多使用 4 输入 LUT，所以每一个 LUT 可以看成一个有 4 位地址线的 16×1 的 RAM 。当用户通过原理图或 HDL 语言描述了一个逻辑电路以后， FPGA 开发软甲会自动计算逻辑电路的所有可能的结果，并把结果事先写入 RAM ，meizu 输入一个信号进行逻辑运算就等于输入一个地址进行查表，找出地址对应的内容，然后输出即可。
