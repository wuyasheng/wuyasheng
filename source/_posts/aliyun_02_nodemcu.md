...
**注意**：编程过程当中不能出现中文，否则可能会下载程序失败，注释也是。
...
wifi.lua 文件内容（连接WIFI）

```lua
print("set up Wifi mode")
wifi.setmode(wifi.STATION) 
station_cfg = {}   
station_cfg.ssid = "216_WIFI" 
station_cfg.pwd = "wuyasheng"  
station_cfg.save = true    
wifi.sta.config(station_cfg)  
wifi.sta.connect()  
wifi_wait = tmr.create()  
...
前提固件当中有ntp模块，并且已经连接至WIFI，即可进行时间网络同步，再使用rtc模块实时获取时间即可

再打印时间之前需要判断时间同步是否完成，否则时间会错误，不过也不影响
...
function time_sync_ntp()
     print("begin to sync time...")
    if isConnect() == true then    
        sntp.sync({"ntp.aliyun.com"}, 
...
ntp_sync = tmr.create()  
ntp_sync:register(2000,tmr.ALARM_AUTO,function()
    if isConnect() == true then
        print('begin to Sync time....')
        ntp_sync:stop()  
...
time_disp = tmr.create()  
time_disp:register(1000,tmr.ALARM_AUTO,function()
...
**关于 tmr.alarm()的使用**，新版固件好像不太支持，还是使用声明、注册、开始、停止的方式进行使用
